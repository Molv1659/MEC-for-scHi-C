...
sdae = StackedDAE(input_dim=150*150, z_dim=30, binary=False,
    ...
    dropout=0)
print(sdae)
sdae.pretrain(train_loader, val_loader, lr=0.0001, batch_size=batch_size, 
    num_epochs=30, corrupt=0.2, loss_type="mse")
sdae.fit(train_loader, val_loader, lr=0.0001, num_epochs=30, corrupt=0.2, loss_type="mse")
sdae_savepath = ("model/sdae_300_30_lr_0001_epoch_30_symmetric_Adam_150_run3.pt")
...
for i in range(len(chrom_matrix)//23):
    z, x = sdae.forward(chrom_matrix_cuda[23*i:23*i+23])
    z = z.view(1, -1)
