...
class DCEC_BN_5(nn.Module):
    def __init__(self,
        ...
        self.mu = nn.Parameter(torch.Tensor(num_clusters, 23 * embedding_dim))
        self.alpha = 1.0
    ...
    def encode(self, x):
        ...
        x = x.view(x.size(0), self.filters[-1], self.conv_sizes[-1],
                    self.conv_sizes[-1])
        if self.check_code:
            print('resize:', x.shape)
        x = self.deconv5(x)
    ...
    def fit(self,
        ...
        if kmeans_init == True:
            ...
            print("Kmeans ACC: %.5f, NMI: %.5f, ARI: %.5f" %
                    (acc(y, y_pred), normalized_mutual_info_score(
                    y, y_pred), adjusted_rand_score(y, y_pred)))
