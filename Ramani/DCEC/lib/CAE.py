...
class CAE_BN_5(nn.Module):
    def __init__(self, input_shape=[250, 250, 1], embedding_dim=30, num_clusters=4, filters=[32, 64, 128, 64, 32], leaky=True, neg_slope=0.01, activations=False, bias=True):
    ...
    def fit(self, chrom_matrix, labels, trainloader, validloader, lr=0.001, num_epochs=10, loss_type="mse"):
        ...
        for epoch in range(num_epochs):
            ...
            for i in range(len(chrom_matrix)//23):
                z, x = self.forward(chrom_matrix_cuda[23*i:23*i+23])
                z = z.view(1, -1)
            ...
            y_pred = KMeans(n_clusters=self.num_clusters, n_init=200).fit_predict(cell_embedding_reduce)
